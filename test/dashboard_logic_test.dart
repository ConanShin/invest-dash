...
class ManualMockStockService extends Mock implements StockService {
  ...
  Future<Map<String, StockPriceData>> getPrices(List<String> symbols) =>
      super.noSuchMethod(
        ...
        returnValue: Future.value(<String, StockPriceData>{}),
      );
...
void main() {
  test(
    ...
    () async {
      ...
      when(mockStockService.getPrices(['AAPL'])).thenAnswer(
        (_) async => {
          'AAPL': StockPriceData(currentPrice: 200.0, previousClose: 190.0),
        },
      );
      ...
      // Initially, tradeable assets (like AAPL) should be 0: (10 * 0) * 1300 = 0
      expect(initialState.totalValue, 0.0);

      // Wait for the state to update via _fetchCurrentPrices
      final completer = Completer<DashboardState>();
      final listener = container.listen(dashboardViewModelProvider, (
        prev,
        next,
      ) {
        if (next.hasValue && next.value!.totalValue == 2600000.0) {
      ...
      final state = await completer.future.timeout(
        ...
        onTimeout: () =>
            throw TimeoutException('Timed out waiting for price update'),
      );
      listener.close();
      ...
      final testAssets = [
        AssetWithHolding(
          ...
          holding: Holding(
            ...
            quantity: 3.5,
            averagePrice: 10000,
          ), // 3.5% rate, 10000 principal
        ),
      ];

      when(mockRepo.getAllAssets()).thenAnswer((_) async => testAssets);
      when(
        mockStockService.getPrices(const []),
      ...
      final testAssets = [
        AssetWithHolding(
          ...
          holding: Holding(
            ...
            quantity: 1000,
            averagePrice: 1.5,
          ), // 1000 units * 1.5 price = 1500 value
        ),
      ];
